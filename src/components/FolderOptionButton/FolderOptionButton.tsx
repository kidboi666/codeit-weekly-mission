...
const FolderOptionButton = () => {
  ...
  return (
    <S.FolderOptionButtonLayout>
      ...
      {folderId && (
        <S.OptionContainer>
          <S.OptionBox onClick={() => dispatch(openModal({ type: 'shareFolder' }))}>
            <Image src={shareIcon} alt={'공유버튼'} />
            공유
          </S.OptionBox>
          <S.OptionBox onClick={() => dispatch(openModal({ type: 'changeName' }))}>
      ...
      {router.pathname === '/paper' && (
        <S.OptionContainer>
          <S.OptionBox onClick={() => dispatch(openModal({ type: 'newPaperForm' }))}>
            ✚ 새 글
          </S.OptionBox>
        </S.OptionContainer>
      )}
